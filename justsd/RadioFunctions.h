...
const int RX_LED = 5;  // PE3 - RF RX LED
const int TX_LED = 2;  // PE4 - RF TX LED
...
uint8_t rfBegin(uint8_t channel)
{
  ...
  if ((channel < 11) || (channel > 26)) channel = 26;  
  PHY_CC_CCA = (PHY_CC_CCA & 0xE0) | 26; // Set the channel to 26
...
// If the buffer is emtpy, it'll return a new line.
char rfRead()
{
...
{
  digitalWrite(RX_LED, LOW);  // Turn receive LED on
...
ISR(TRX24_RX_END_vect)
{
  ...
  digitalWrite(RX_LED, HIGH);  // Turn receive LED off, and we're out
}
